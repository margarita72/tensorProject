...
var modelBuilder = /** @class */ (function () {
    ...
    function modelBuilder(keys, values) {
        ...
        var _loop_1 = function (i_1) {
            ...
            Object.defineProperty(this_1, properties[i_1], {
                set: function (val) { this['_' + properties[i_1]] = val; },
                get: function () { return this['_' + properties[i_1]]; }
            });
        };
        var this_1 = this;
        for (var i_1 = 0; i_1 < properties.length; i_1++) {
            _loop_1(i_1);
        }
        if (keys && values) //Если массив ключей и значений не был передан, то создаем пустой объект. 
         {
            ...
            var i_2 = 0;
            for (; i_2 < keys.length; i_2++) { //Цикл обработки элементов массива.
                res[keys[i_2]] = values[i_2]; //Создается новое поле с названием key[i] и с значением values[i].
            }
        }
    }
    /**
     ...
     * @name delete
     */
    modelBuilder.prototype["delete"] = function () {
     ...
     */
    modelBuilder.prototype.update = function (updObj) {
     ...
     */
    modelBuilder.prototype.read = function (key) {
        ...
        if (key) //Если был передан ключ, то функция должна вернуть значение по ключу.
         {
            if (this[key])
                ...
                return null; //Если ключа нет или значение undefined, возвращает null.
        }
     ...
     */
    modelBuilder.prototype.getChildren = function (callback) {
        if (this.hasChildren) //Если у списка есть дочерний список то открывается.
         {
            var getAsyncChildren = function (_id) {
                return new Promise(function (resolve) {
                    resolve(_id);
                });
            };
     ...
     * @function
     */
    modelBuilder.prototype.changeDone = function () {
        this.done = !this.done;
    };
    return modelBuilder;
}());
...
function openLine(_event) {
    ...
    var currentObj = dataList[pressedButton.id]; //Получаем ссылку на объект modelBuilder
...
function openTasks(_event) {
    var pressedButton = _event.currentTarget; //Получаем html кнопки на которую нажали.
    var currentObj = dataList[pressedButton.id]; //Получаем ссылку на объект modelBuilder
    var currentHTML = $('#list' + currentObj.id);
    ...
    if (!dataList['btn' + currentObj.id].opened) {
        ...
        var title = $('#btn' + currentObj.id).clone(); //Сохраняем заголовок в переменную.
